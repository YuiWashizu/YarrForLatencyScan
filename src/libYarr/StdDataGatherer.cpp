...
void StdDataGatherer::execPart2() {
	...
	count_sec ++;
	// std::cout << "count sec : " << count_sec << std::endl;
		...
		newData = NULL;	       
		past_eve ++;
		// std::cout << "count = " << past_eve << std::endl;
            }
            std::this_thread::sleep_for(std::chrono::microseconds(100));
        } while (newData != NULL && signaled == 0 && !killswitch );// past_eve < 100);
        if (newData != NULL)
		delete newData;
        rdc->stat = *g_stat;	
        storage->pushData(std::move(rdc));
        if (signaled == 1 || killswitch || past_eve == 100) {//count_sec == 4e4) {//past_eve == 400) {
