...
void Rd53aGlobalFeedback::execPart1() {
  std::cout << "Rd53aGlobalFeedback::execPart1()" << std::endl;
...
void Rd53aGlobalFeedback::execPart2() {
  ...
  std::cout << "Rd53aGlobalFeedback::execPart2()" << std::endl;
    for (auto fe: keeper->feList) {
      std::cout << "in for " << std::endl;
        if (fe->getActive()) {
	  std::cout << "in if " << std::endl;
            unsigned rx = dynamic_cast<FrontEndCfg*>(fe)->getRxChannel();
	    std::cout << "befor mutex" << std::endl;
            keeper->mutexMap[rx].lock();
	    std::cout << "before m_values[rx] = " << m_values[rx] << std::endl;
	    int val = (m_values[rx]-(m_localStep[rx]));
	    if (val > (int)max) val = max;
	    if (val < min) val = min;
	    m_values[rx] = val;
	    ...
	    std::cout << "after  m_values[rx] = " << m_values[rx] << std::endl;
            std::cout << " --> Received Feedback on Channel " << rx << " with value: " << m_values[rx] << std::endl;
	    keeper->mutexMap[rx].unlock();
